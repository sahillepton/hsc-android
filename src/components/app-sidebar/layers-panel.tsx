            ...
            {layers &&
              layers.map((layer) => {
                const geojsonFeatures =
                  layer.type === "geojson" ? layer.geojson?.features ?? [] : [];
                const hasGeoJsonLine =
                  layer.type === "geojson" &&
                  geojsonFeatures.some((feature) => {
                    const type = feature.geometry?.type;
                    return type === "LineString" || type === "MultiLineString";
                  });
                const hasGeoJsonPoint =
                  layer.type === "geojson" &&
                  geojsonFeatures.some((feature) => {
                    const type = feature.geometry?.type;
                    return (
                ...
                return (
                  <DropdownMenu>
                          ...
                          <div className="flex flex-col items-start">
                            ...
                            {layer.type === "polygon" && layer.polygon ? (
                              <span className="text-xs text-muted-foreground">
                                {layer.sectorAngleDeg !== undefined
                                  ? `${layer.sectorAngleDeg}°`
                    ...
                    <DropdownMenuContent className="w-72" align="start">
                      ...
                      {layer.type === "geojson" && hasGeoJsonLine && (
                        <>
                          <div className="p-3 space-y-3">
                            <label className="text-xs font-medium text-muted-foreground block">
                              Line Width
                      ...
                      {layer.type === "geojson" && hasGeoJsonPoint && (
                        <div className="p-3 space-y-3">
                          <label className="text-xs font-medium text-muted-foreground block">
